...
Projekt by³ dla mnie wymagaj¹cy i ciekawy, poniewa¿ pozwoli³ mi zmierzyæ siê z analiz¹ tak du¿ych danych. Najwiêcej problemów przystworzy³o mi ci¹g³e dostosowywanie danych tak, aby wystarczy³o mi pamiêci operacyjnej. Wykona³em wiêkszoœæ z postawionych zadañ, ale mia³em te¿ problemy mniejsze lub wiêksze. Najtrudniejsza by³a dla mnie budowa klasyfikatora i niestety do tej pory nie dzia³a on najlepiej. 
...
k <- top50 %>% select(local_res_atom_non_h_count, dict_atom_non_h_count, local_res_atom_non_h_electron_sum, dict_atom_non_h_electron_sum) 

kor <- cor(k)

kor_melt <- melt(kor)

kor_gg <- ggplot(kor_melt, aes(Var1, Var2, fill = value)) + geom_tile() + theme(axis.text.x = element_text(angle = 90))

kor_plt <- ggplotly(kor_gg)
...
#ggplot(part01, aes(nazwa, wartosc)) + geom_boxplot()
...
# 
# data_top50_k <- top50 %>% select(-usun) %>% head(10000)
# 
